...
class KsProjectTask(models.Model):
    ...
    def create(self, vals_list):
        result = super(KsProjectTask, self).create(vals_list)
        ...
        for res in result:
            ...
            if res.ks_task_duration and res.ks_enable_task_duration:
                ...
                if ids:
                    ...
                    self.ks_end_datetime = self.ks_calculate_end_date(res.ks_task_duration, res.ks_start_datetime, day_integers)
                else:
                    res.ks_end_datetime = res.ks_start_datetime + timedelta(days=res.ks_task_duration)

            # if the task is in the auto mode and constr
            if res.ks_schedule_mode == 'auto' and res.ks_constraint_task_type in ('asap', 'alap'):
