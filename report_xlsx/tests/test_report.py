...
class TestReport(common.TransactionCase):
    ...
    def test_render_with_report_file_ref(self):
        rep = self.report_object._render_xlsx(self.report.report_file, self.docs.ids, {})
        file = io.BytesIO(rep[0])
        wb = load_workbook(file)
        sheet = wb.active
        self.assertEqual(sheet.cell(1, 1).value, self.docs.name)

    def test_render_with_report_record_ref(self):
        rep = self.report_object._render_xlsx(self.report, self.docs.ids, {})
        file = io.BytesIO(rep[0])
        wb = load_workbook(file)
        sheet = wb.active
        self.assertEqual(sheet.cell(1, 1).value, self.docs.name)
