      ...
      let ks_gantt_config_columns = [
        {
          ...
          template: function (ks_task) {
            if (["task", "milestone"].indexOf(ks_task.type) > -1) {
              const progress = ks_task.ks_progress * 100
              if (progress == 0) {
                // Red
        ...
        },
        {
          ...
          template: function (obj) {
            ...
            return ks_column_str + escape(obj.text);
          },
        },
        {
          name: "duration",
          ...
          resize: true,
          width: 95,
          template: (task) => this.calcTaskDuration(task),
        },
        {
          name: "start_date",
          align: "center",
...
//       Added grid button.
    ...
    calcTaskDuration(task){
        if (task.unscheduled) return ""

        let difference = "";
        let milliseconds = task.end_date - task.start_date;
        let hours = Math.floor(milliseconds / 1000 / 60 / 60);

        let days = Math.floor(hours / 24);
        let ks_rem_hours = hours % 24;
        let ks_minute = Math.floor(milliseconds / 60000) % 60;
        let ks_seconds = (milliseconds / 1000) % 60;

        if (days) difference += days + _t(" day, ");

        difference += (ks_rem_hours < 10 ? "0" + ks_rem_hours : ks_rem_hours) + ":";
        difference += ks_minute < 10 ? "0" + ks_minute : ks_minute;
        task.ks_task_difference = difference;
        task.ks_task_duration = difference;
        return difference;
    }
...
//                var ks_new_project_id = this.props.model.ksDataForGantt.groupedBy[ks_groupb_date].split(':')[0]
    ...
    ks_data_update(each_task,ks_links,ks_data,ks_gantt_fields,parent_group_id) {
      ...
      var ks_gantt_data = {
        ...
        text: each_task[escape(ks_gantt_fields.ks_task_name)],
...
//       Check if parent task is available then set add parent otherwise set to false.
    ...
    ks_project_task_data_update(
      ...
      var ks_gantt_datas = {
        ...
        text: escape(each_project_task.ks_task_name),
...
//                        'ks_allow_parent_task': each_project_task.ks_allow_subtask,
        ...
        ks_task_duration: each_project_task.ks_task_duration || this.calcTaskDuration(each_project_task) ,
        open: true,
      };
      // Manage multiple users.
      // if (
